...
export default function HomeCorrigida() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0e1a', color: 'white' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 16 }}>
        <div style={{ textAlign: 'center', margin: '20px 0' }}><h2 style={{ fontSize: 24, fontWeight: 800 }}>ELEITORAL 2026</h2><p style={{ opacity: 0.5, fontSize: 13 }}>Protótipo Completo</p></div>
        <div style={{ maxWidth: 600, margin: '0 auto 14px', position: 'relative' }}>
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar..." style={{ width: '100%', padding: '12px 14px 12px 40px', borderRadius: 12, background: '#111827', border: '1px solid #1e293b', color: 'white', outline: 'none' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: 16 }}>
          {filtrados.map(c => (
            <div key={c.id} className="card" style={{ background: '#151a29', border: '1px solid #1e293b', borderRadius: 16, overflow: 'hidden', transition: '0.2s' }}>
              {/* AQUI ESTÁ A CORREÇÃO: /candidato?id= */}
