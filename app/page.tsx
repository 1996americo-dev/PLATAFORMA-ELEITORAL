...
const CODIGOS_VALIDOS = ["GRAZI2026", "ELEICAO2026", "PLATAFORMA", "LIBERADO"] // você pode trocar
...
export default function Home(){
  ...
  const [mostrarPix, setMostrarPix] = React.useState(false)
  ...
  function abrirWhatsApp(){
    const msg = `Olá Grazi! Paguei o PIX da Plataforma Eleitoral 2026 - ${VALOR}. PIX: ${PIX}. Meu comprovante está aqui. Pode liberar meu acesso?`
    window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(msg)}`, "_blank")
  }

  function liberarAcesso(){
    const codigo = codigoAcesso.trim().toUpperCase()
    // Aceita códigos pré-definidos OU qualquer código no formato ADM-XXXX que você enviar
    ...
    if(ehValido){
      ...
      // Gera senha do admin automaticamente
      ...
      alert(`✅ ACESSO LIBERADO!\n\nSua senha ADMIN é: ${senhaAdmin}\n\nAnote! Use no botão ADMIN.\n\nBem-vinda à Plataforma!`)
    } else {
      alert("❌ CÓDIGO INVÁLIDO!\n\nVocê ainda não pagou ou digitou errado.\n\n1. Faça o PIX\n2. Envie comprovante no WhatsApp\n3. A Grazi vai te enviar o código de liberação!")
    }
  }

  function getFoto(url:string){
  ...
  // TELA DE PAYWALL - BLOQUEIO TOTAL
  if(!acessoLiberado){
    return (
      <div style={{background:"black",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"system-ui"}}>
        <div style={{background:"white",borderRadius:20,padding:25,maxWidth:420,width:"100%",border:"5px solid #facc15",textAlign:"center"}}>
          ...
          <div style={{background:"#f5f5f5",border:"3px dashed black",borderRadius:12,padding:15,marginTop:15,textAlign:"left"}}>
            ...
            <div style={{marginTop:10,fontSize:12,fontWeight:700}}>Nome: Grazi Ameri • Banco: Qualquer banco aceita PIX por celular</div>
          </div>
          ...
          <div style={{marginTop:15,fontSize:10,color:"#666",fontWeight:700}}>
            Como funciona: 1. Copia o PIX 2. Paga R$97 3. Clica no botão do WhatsApp e manda o comprovante 4. Grazi te manda o código em 2 min 5. Digita o código aqui e entra!
          </div>
        </div>
      </div>
    )
  ...
  // SITE LIBERADO - SÓ APARECE DEPOIS DE PAGAR
  return (
    <div style={{background:"#FFFFFF",minHeight:"100vh",padding:20,fontFamily:"system-ui"}}>
      <div style={{background:"black",border:"4px solid black",borderRadius:16,padding:"18px",marginBottom:20,textAlign:"center",position:"relative"}}>
        ...
        <button onClick={()=>{localStorage.removeItem("acesso_liberado_2026"); setAcessoLiberado(false)}} style={{position:"absolute",top:10,right:10,background:"white",border:"2px solid black",borderRadius:6,padding:"2px 6px",fontSize:9,fontWeight:900}}>SAIR</button>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:20}}>
        <div style={{background:"#FFFFFF",border:"5px solid black",borderRadius:16,padding:16}}>
          ...
          <div style={{border:"3px dashed black",borderRadius:12,padding:12,marginTop:10,minHeight:70}}>
            {votos.length===0 ? <div style={{textAlign:"center",padding:10,fontWeight:900}}>Nenhum voto ainda - Seja o primeiro!</div> : rankingCalculado.map((c:any,i:number)=><div key={c.id} style={{display:"flex",justifyContent:"space-between",background:i===0?"#facc15":"#e5e7eb",padding:"10px 12px",borderRadius:8,marginBottom:6,border:"3px solid black",fontWeight:900}}><span>{i+1}º {c.nome} - {c.sigla}</span><span>{c.qtd} votos - {((c.qtd/votos.length)*100).toFixed(1)}%</span></div>)}
          </div>
          <h3 style={{fontWeight:900,margin:"20px 0 10px 0"}}>TODOS CANDIDATOS - {cands.length} CANDIDATOS</h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {cands.map((c:any)=>{
              const qtd=votos.filter((v:any)=>v.candidato_id===c.id).length
              return <div key={c.id} style={{border:"4px solid black",borderRadius:12,padding:12}}><div style={{display:"flex",gap:10,alignItems:"center"}}><img src={getFoto(c.foto_url)} style={{width:45,height:45,borderRadius:"50%",border:"2px solid black"}} alt=""/><div><b style={{fontSize:13}}>{c.nome}</b><br/><span style={{background:"black",color:"white",padding:"2px 6px",borderRadius:4,fontSize:10,fontWeight:900}}>{c.sigla}</span></div></div>{qtd>0 && <div style={{marginTop:8,fontWeight:900,textAlign:"center",background:"#facc15",borderRadius:20,padding:6,border:"2px solid black",fontSize:12}}>{qtd} votos</div>}</div>
            })}
          </div>
        </div>
        <div style={{background:"#FFFFFF",border:"5px solid black",borderRadius:16,padding:16,height:"fit-content"}}>
          ...
          <button onClick={()=>window.location.href="/admin"} style={{width:"100%",marginTop:10,background:"white",color:"black",fontWeight:900,padding:10,borderRadius:12,border:"3px solid black"}}>ADMIN - SENHA: {typeof window!=="undefined" ? localStorage.getItem("senha_admin_gerada") : ""}</button>
          {cpfLiberado && <><div style={{marginTop:15,background:"#22c55e",color:"white",padding:10,borderRadius:10,textAlign:"center",fontWeight:900,border:"3px solid black"}}>✅ LIBERADO! Vote agora:</div><div>{cands.map((c:any)=><button key={c.id} onClick={()=>votar(c.id)} style={{width:"100%",marginTop:6,background:"#facc15",border:"3px solid black",padding:8,borderRadius:8,fontWeight:900,fontSize:12,cursor:"pointer"}}>VOTAR {c.nome}</button>)}</div></>}
        </div>
      </div>
    </div>
  )
