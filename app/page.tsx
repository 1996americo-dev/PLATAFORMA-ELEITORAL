...
export default function Page(){
  ...
  const getFotoUrl = (f:string) => {
    if(!f) return "https://via.placeholder.com/200"
  ...
  const votar=async()=>{if(!sel){alert("Escolha um candidato!");return};const {error}=await supabase.from('votos').insert({candidato_id:sel,cpf_hash:hash});if(error){alert(error.message);return};alert("VOTO COMPUTADO COM SUCESSO!");setCpf("");setOk(false);setSel("");setHash("");load()}
    ...
    <div style={{minHeight:'100vh', background:'#020617', color:'white', padding:12, fontFamily:'system-ui'}}>
      <div style={{maxWidth:1220, margin:'0 auto'}}>
        <div style={{background:'#FFFFFF', border:'4px solid black', borderRadius:16, padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow:'0 6px 0 black'}}>
          <div style={{color:'black', fontWeight:900, fontSize:12}}>V12 FINAL • BUCKET fotos-candidatos • {cands.length} CANDIDATOS • {total} VOTOS</div>
            ...
            <div style={{marginTop:12, display:'grid', gap:10}}>
              ...
              {rank.map(([id,qtd]:any,i)=>{
                ...
                <div key={id} style={{background:i===0?'#020617':'#FFFFFF', color:i===0?'white':'black', border:'3px solid black', borderRadius:14, padding:12, display:'flex', alignItems:'center', gap:12, boxShadow:'0 4px 0 black'}}>
                  ...
                  <div style={{flex:1, minWidth:0}}><div style={{fontWeight:900, fontSize:13, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{c?.nome||id.slice(0,8)} <span style={{fontSize:10, background:'black', color:'white', padding:'2px 6px', borderRadius:999, marginLeft:6}}>{c?.partido} {c?.numero}</span></div><div style={{fontSize:11, fontWeight:700}}>{qtd} votos</div></div>
                  <div style={{fontWeight:900, fontSize:18}}>{total?((qtd/total)*100).toFixed(1):0}%</div>
                </div>
              )})}
            </div>
            <div style={{marginTop:20, color:'black', fontWeight:900, fontSize:14, borderTop:'4px solid black', paddingTop:10}}>TODOS CANDIDATOS - {cands.length} DO SEU BUCKET fotos-candidatos</div>
              ...
              {cands.map(c=>
                <button key={c.id} onClick={()=>ok&&setSel(c.id)} disabled={!ok} style={{border:`${sel===c.id?'5px':'4px'} solid black`, borderRadius:16, padding:12, background:sel===c.id?'#020617':'#FFFFFF', color:sel===c.id?'white':'black', fontWeight:900, textAlign:'left', cursor:ok?'pointer':'not-allowed', opacity:ok?1:0.6, boxShadow:'0 5px 0 black'}}>
                  <div style={{display:'flex', gap:10, alignItems:'center'}}>
                    <div style={{width:56, height:56, borderRadius:14, background:'#E2E8F0', border:'3px solid black', overflow:'hidden', flexShrink:0}}><img src={getFotoUrl(c.foto_url)} alt={c.nome} style={{width:'100%', height:'100%', objectFit:'cover'}}/></div>
                    <div style={{minWidth:0}}><div style={{fontSize:13, fontWeight:900, lineHeight:1.1}}>{c.nome}</div><div style={{fontSize:11, fontWeight:800}}>{c.partido} • {c.numero}</div></div>
                  </div>
                </button>
              )}
            </div>
