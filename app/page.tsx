...
export default function Page(){
  ...
  const getFotoUrl = (fileName:string) => {
    if(!fileName) return "https://via.placeholder.com/100"
    if(fileName.startsWith('http')) return fileName
    const {data} = supabase.storage.from('candidatos-fotos').getPublicUrl(fileName.trim())
  ...
  return(
    <div style={{minHeight:'100vh', background:'#020617', color:'white', padding:12}}>
      <div style={{maxWidth:1220, margin:'0 auto'}}>
        <div style={{background:'#FFF', border:'4px solid black', borderRadius:16, padding:10, display:'flex', justifyContent:'space-between', boxShadow:'0 6px 0 black'}}>
          <div style={{color:'black', fontWeight:900, fontSize:12}}>V10 BUCKET REAL - {cands.length} CANDIDATOS - FOTO DO SUPABASE</div>
          ...
          <div style={{background:'#FFF', border:'4px solid black', borderRadius:20, padding:16, boxShadow:'0 8px 0 black'}}>
            ...
            <div style={{marginTop:12, display:'grid', gap:10}}>
              ...
              {rank.map(([id,qtd]:any,i)=>{
                ...
                return(
                <div key={id} style={{background:i===0?'#020617':'#FFF', color:i===0?'white':'black', border:'3px solid black', borderRadius:14, padding:12, display:'flex', alignItems:'center', gap:12, boxShadow:'0 4px 0 black'}}>
                  ...
                  <div style={{width:48, height:48, borderRadius:999, border:'3px solid black', overflow:'hidden', background:'#EEE'}}><img src={c?getFotoUrl(c.foto_url):''} style={{width:'100%', height:'100%', objectFit:'cover'}}/></div>
                  <div style={{flex:1}}><div style={{fontWeight:900, fontSize:13}}>{c?.nome||id}</div><div style={{fontSize:11}}>{qtd} votos</div></div>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:10}}>
              {cands.map(c=>
                <button key={c.id} onClick={()=>ok&&setSel(c.id)} disabled={!ok} style={{border:`${sel===c.id?'5px':'4px'} solid black`, borderRadius:16, padding:12, background:sel===c.id?'#020617':'#FFF', color:sel===c.id?'white':'black', fontWeight:900, textAlign:'left', cursor:ok?'pointer':'not-allowed', opacity:ok?1:0.6, boxShadow:'0 5px 0 black'}}>
                  <div style={{display:'flex', gap:10, alignItems:'center'}}>
                    <div style={{width:56, height:56, borderRadius:14, border:'3px solid black', overflow:'hidden', background:'#EEE'}}><img src={getFotoUrl(c.foto_url)} style={{width:'100%', height:'100%', objectFit:'cover'}}/></div>
                    <div><div style={{fontSize:13}}>{c.nome}</div><div style={{fontSize:11}}>{c.partido} {c.numero}</div></div>
                  </div>
                </button>
              )}
            </div>
          </div>
          <div style={{background:'#FFF', border:'4px solid black', borderRadius:20, padding:16, boxShadow:'0 8px 0 black', height:'fit-content'}}>
