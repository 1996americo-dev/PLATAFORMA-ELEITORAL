...
export default function Admin(){
  ...
  const [nome,setNome]=useState(""); const [numero,setNumero]=useState(""); const [partido,setPartido]=useState(""); const [cargo,setCargo]=useState("Presidente"); const [cargoOutro,setCargoOutro]=useState(""); const [propostas,setPropostas]=useState(""); const [fotoFile,setFotoFile]=useState<File|null>(null); const [uploading,setUploading]=useState(false);
  ...
  async function cadastrar(e:any){
    e.preventDefault();
    const cargoFinal = cargo==="Outro" ? cargoOutro : cargo;
    if(!nome||!numero||!partido||!cargoFinal){ alert("Preencha tudo! Se escolheu Outro, digite o cargo."); return; }
    ...
    try{
      ...
      let {error}=await sb.from("candidatos").insert({nome,numero,partido,cargo:cargoFinal,propostas,foto_url:fotoNome});
      if(error && error.message.toLowerCase().includes("cargo")){
        const res=await sb.from("candidatos").insert({nome,numero,partido,propostas:`[${cargoFinal}] ${propostas}`,foto_url:fotoNome});
        if(res.error) throw res.error;
        setMsg(`✅ ${nome} para ${cargoFinal} cadastrado! Crie coluna cargo no Supabase.`);
      } else { if(error) throw error; setMsg(`✅ ${nome} para ${cargoFinal} cadastrado!`); }
      setNome(""); setNumero(""); setPartido(""); setPropostas(""); setCargo("Presidente"); setCargoOutro(""); setFotoFile(null); (document.getElementById("fotoInput") as any).value="";
      load();
  ...
  return (
    <div style={{padding:20,fontFamily:"system-ui",background:"#FFFFFF",minHeight:"100vh"}}>
      ...
      <div style={{background:"#facc15",border:"5px solid black",borderRadius:16,padding:20,marginBottom:20,boxShadow:"0 8px 0 black"}}>
        <h2 style={{fontWeight:900,margin:0,color:"black"}}>➕ CADASTRAR CANDIDATO - UNIVERSAL</h2>
        <form onSubmit={cadastrar} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:15,marginTop:15}}>
          ...
          <div>
            <label style={{fontWeight:900,fontSize:12}}>CARGO *</label>
            ...
            {cargo==="Outro" && <input value={cargoOutro} onChange={e=>setCargoOutro(e.target.value)} placeholder="Digite o cargo: Ex: Líder Comunitário" style={{width:"100%",border:"3px solid black",borderRadius:10,padding:12,fontWeight:800,background:"white",marginTop:8}} required/>}
          </div>
          <div><label style={{fontWeight:900,fontSize:12}}>PARTIDO / CHAPA / TURMA *</label><input value={partido} onChange={e=>setPartido(e.target.value)} placeholder="Ex: PL, Chapa 1, 3ºA" style={{width:"100%",border:"3px solid black",borderRadius:10,padding:12,fontWeight:800,background:"white"}} required/></div>
          <div style={{gridColumn:"1 / 3"}}><label style={{fontWeight:900,fontSize:12}}>PROPOSTAS</label><textarea value={propostas} onChange={e=>setPropostas(e.target.value)} placeholder="Propostas..." style={{width:"100%",border:"3px solid black",borderRadius:10,padding:12,fontWeight:700,background:"white",minHeight:70}} /></div>
          <div style={{gridColumn:"3 / 5"}}><label style={{fontWeight:900,fontSize:12}}>FOTO</label><input id="fotoInput" type="file" accept="image/*" onChange={e=>setFotoFile(e.target.files?.[0]||null)} style={{width:"100%",border:"3px solid black",borderRadius:10,padding:10,background:"white",fontWeight:700}}/><button type="submit" disabled={uploading} style={{width:"100%",marginTop:10,background:"black",color:"#facc15",border:"3px solid black",borderRadius:12,padding:12,fontWeight:900,cursor:"pointer"}}>{uploading?"SALVANDO...":`CADASTRAR PARA ${(cargo==="Outro"?cargoOutro: cargo).toUpperCase()||"CARGO"}`}</button></div>
        </form>
      </div>
