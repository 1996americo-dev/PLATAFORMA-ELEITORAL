...
export default function VendasPage(){
  ...
  React.useEffect(()=>{
    // VENDAS TAMBÉM SEMPRE PEDE SENHA
      ...
      <div style={{minHeight:"100vh",background:"#0f172a",display:"flex",alignItems:"center",justifyContent:"center",padding:16, fontFamily:"system-ui"}}>
        <div style={{background:"white",borderRadius:20,padding:28,width:400,border:"4px solid black", boxShadow:"8px 8px 0px #000"}}>
          <div style={{textAlign:"center"}}>
            <div style={{width:70,height:70, background:"#bbf7d0", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto", fontSize:32, border:"4px solid black"}}>💰</div>
          ...
          <button onClick={liberar} style={{width:"100%",marginTop:10,background:"black",color:"#22c55e",padding:14,borderRadius:12,fontWeight:900,border:"4px solid black", boxShadow:"4px 4px 0px #000", fontSize:13}}>ENTRAR EM VENDAS →</button>
    ...
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui"}}>
      <div style={{background:"linear-gradient(90deg,#0f172a,#1e293b)",color:"white",padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"4px solid black"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{background:"#22c55e",color:"black",width:40,height:40,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:14,border:"3px solid black", boxShadow:"3px 3px 0px #000"}}>$</div>
          <div>
            <div style={{fontWeight:900,fontSize:15}}>VENDAS • 2026</div>
            <div style={{fontSize:11,background:"white",color:"black",padding:"2px 8px",borderRadius:20,display:"inline-block",marginTop:3,border:"2px solid black",fontWeight:700}}>{codigos.length} códigos gerados</div>
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <a href="/admin" style={{background:"white",color:"black",padding:"8px 14px",borderRadius:10,fontSize:11,fontWeight:900,textDecoration:"none",border:"3px solid black", boxShadow:"3px 3px 0px #000"}}>Admin</a>
      ...
      <div style={{maxWidth:800,margin:"0 auto",padding:16,display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:"white",borderRadius:16,border:"4px solid black",padding:18, boxShadow:"6px 6px 0px #000"}}>
          <div style={{fontWeight:900,fontSize:14,background:"black",color:"#22c55e",padding:"8px 14px",borderRadius:10,display:"inline-block",border:"3px solid black"}}>💰 GERAR CÓDIGO CLIENTE</div>
          <p style={{fontSize:12,marginTop:10,fontWeight:600}}>Cada código LIBERADO-XXXX libera 1 cliente no site principal E serve pra você entrar no admin dele.</p>
          <button onClick={gerar} style={{width:"100%",marginTop:12,background:"#22c55e",color:"black",border:"4px solid black",borderRadius:12,padding:14,fontWeight:900,fontSize:14,cursor:"pointer",boxShadow:"4px 4px 0px #000"}}>GERAR NOVO CÓDIGO +</button>
        </div>
        ...
        <div style={{background:"white",borderRadius:16,border:"4px solid black",padding:18, boxShadow:"6px 6px 0px #000"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:12}}>
            {codigos.length===0?<div style={{textAlign:"center",padding:20,fontWeight:900,fontSize:13}}>Nenhum código ainda - clique em GERAR</div>:
              codigos.map((c,i)=>(
              <div key={i} style={{border:"3px solid black",background:"#f8fafc",borderRadius:12,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center", boxShadow:"3px 3px 0px #000"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <span style={{background:"black",color:"#22c55e",padding:"6px 12px",borderRadius:8,fontWeight:900,fontSize:13,border:"2px solid black"}}>{c}</span>
                  <span style={{fontSize:11,fontWeight:700}}>Usa no principal e no admin</span>
                </div>
                <div style={{display:"flex",gap:6}}>
                  <button onClick={()=>copiar(c)} style={{background:"white",border:"3px solid black",borderRadius:8,padding:"6px 10px",fontWeight:900,fontSize:11,cursor:"pointer"}}>COPIAR</button>
                  <button onClick={()=>remover(i)} style={{background:"#fee2e2",color:"#dc2626",border:"3px solid black",width:28,height:28,borderRadius:8,fontWeight:900,cursor:"pointer"}}>X</button>
                </div>
              </div>
            ))}
          </div>
